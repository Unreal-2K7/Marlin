...
static float bed_safe_z = 20; //used for inital bed probe safe distance (to avoid crashing into bed)
...
//wrapper to be able to call this at the end of G28 - for repetier host compatibility
void print_current_position()
{
      ...
      /* modified to be interpretable by repetier host
...
do
  ...
  //Adjustment complete?
  if ((bed_level_o > bed_level - 0.03) and (bed_level_o < bed_level + 0.03)) adj_done = true;
...
float adj_diagrod_length()
{
  ...
  do {
     ...
     if (bed_level_c - 0.03 < target) adj_val = -adj_mag;
     if (bed_level_c + 0.03 > target) adj_val = adj_mag;
	...
	adj_mag = adj_mag / 2;
     ...
     if ((bed_level_c - 0.03 < target) and (bed_level_c + 0.03 > target)) adj_val = 0;
...
float z_probe() {
  ...
  SERIAL_ECHO(" x="); SERIAL_PROTOCOL_F(destination[X_AXIS],4);
  SERIAL_ECHO(" y="); SERIAL_PROTOCOL_F(destination[Y_AXIS],4);
  SERIAL_ECHO(" z="); SERIAL_PROTOCOL_F(destination[Z_AXIS],4);
  SERIAL_ECHOLN("");*/
...
void calibration_report()
  {
  //Display Report
  SERIAL_ECHO("|\t\tZ-Tower\t\t\tEndstop Offsets  ");
...
{
  /* these were for cartesian movement, i will implement my checking for delta system - unreal.2k7 */
  ...
  if (sq(target[X_AXIS]) + sq(target[Y_AXIS]) > sq(BED_DIAMETER/2))
  {
    SERIAL_ECHOLN(BED_DIAMETER);
    SERIAL_ECHOLN("current:");
    SERIAL_ECHOLN(target[X_AXIS]);
    SERIAL_ECHOLN(target[Y_AXIS]);
    
    float alpha = atan(target[Y_AXIS]/target[X_AXIS]);
    if (target[X_AXIS] < 0) alpha += PI;
    target[X_AXIS] = BED_DIAMETER*cos(alpha);
    target[Y_AXIS] = BED_DIAMETER*sin(alpha);
    
    SERIAL_ECHOLN("would be");
    SERIAL_ECHOLN((BED_DIAMETER/2)*cos(alpha));
    SERIAL_ECHOLN((BED_DIAMETER/2)*sin(alpha));
...
void calculate_delta(float cartesian[3]) 
{
  ...
  /*SERIAL_ECHOLN(delta[X_AXIS]); SERIAL_ECHOLN(delta[Y_AXIS]); SERIAL_ECHOLN(delta[Z_AXIS]);*/
